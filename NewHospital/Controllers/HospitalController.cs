...
namespace NewHospital.Controllers
{
    ...
    public class HospitalController : ControllerBase
    {
        ...
        [HttpPost("timedata")]
        public async Task<IActionResult> PostTimeData([FromBody] TimeDataModel timeData)
        {
            try
            {
                if (timeData == null)
                {
                    return BadRequest("Time data is null.");
                }

                _hospitalDbcontext.TimeData.Add(timeData);
                await _hospitalDbcontext.SaveChangesAsync();

                return Ok("Time data received and saved successfully.");
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("timedata")]
        public async Task<IActionResult> DataTableTaken()
        {
